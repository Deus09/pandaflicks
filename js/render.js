...
import { TMDB_IMAGE_BASE_URL_W185, TMDB_IMAGE_BASE_URL_W92, TMDB_IMAGE_BASE_URL_W500, fetchMovieDetailsFromApi } from './api.js';
...
export function createStarsForDisplay(rating) {
    ...
    const star_svg_path = "M12 .587l3.668 7.568 8.332 1.151-6.064 5.828 1.48 8.279-7.416-3.921-7.417 3.921 1.481-8.279-6.064-5.828 8.332-1.151z";
...
export function renderWatchLaterMovies(listElement, movies, emptyMessageElement, openMovieModeFunction) {
    listElement.innerHTML = '';
    if (movies.length === 0) {
        emptyMessageElement.style.display = 'block';
        return;
...
export function renderTrendingMovies(trendingMovies, openDetailModalFunction) {
    ...
    trendingMovies.forEach(movie => {
        ...
        trendingItem.innerHTML = `<img src="${movie.poster_path ? TMDB_IMAGE_BASE_URL_W185 + movie.poster_path : 'https://placehold.co/80x120/2A2A2A/AAAAAA?text=Poster+Yok'}" alt="${movie.title}" class="trending-poster" onerror="this.onerror=null;this.src='https://placehold.co/80x120/2A2A2A/AAAAAA?text=Poster+Yok';"><div class="trending-title">${movie.title}</div>`;
        trendingItem.addEventListener('click', () => openDetailModalFunction(movie.id));
...
export function displayTmdbSearchResults(results, resultsDiv) {
    ...
    results.slice(0, 5).forEach(movie => {
        ...
        movieResultItem.innerHTML = `<img src="${movie.poster_path ? TMDB_IMAGE_BASE_URL_W92 + movie.poster_path : 'https://placehold.co/40x60/2A2A2A/AAAAAA?text=Yok'}" alt="${movie.title}" class="tmdb-result-poster"><div class="tmdb-result-details"><div class="tmdb-result-title">${movie.title}</div><div class="tmdb-result-year">${movie.release_date ? movie.release_date.substring(0, 4) : ''}</div></div>`;
        ...
        movieResultItem.addEventListener('click', async () => {
            ...
            try {
                const { movieData, directorName } = await fetchMovieDetailsFromApi(movie.id);
                movieTitleInput.value = movieData.title;
                moviePosterInput.value = movieData.poster_path ? TMDB_IMAGE_BASE_URL_W92 + movieData.poster_path : '';
                ...
                if (!movieDateInput.value) movieDateInput.value = movieData.release_date || new Date().toISOString().split('T')[0];
...
export function renderProfilePage(stats) {
    ...
    if (stats.topRatedMovie && stats.topRatedMovie.poster) {
        const posterPath = stats.topRatedMovie.poster.replace(TMDB_IMAGE_BASE_URL_W92, '');
    ...
    const overviewData = [
        { icon: `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5"/></svg>`, value: stats.totalMovies, label: 'Toplam Film' },
        { icon: `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"></polygon></svg>`, value: stats.avgRating.toFixed(1), label: 'Puan Ortalaması' },
        { icon: `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></svg>`, value: stats.totalWatchTime, label: 'Toplam Süre' },
    ];
    ...
    const totalRated = Object.values(stats.ratingDistribution).reduce((sum, count) => sum + count, 0);
    if (totalRated > 0) {
        ...
        const sortedRatings = Object.keys(stats.ratingDistribution).sort((a, b) => parseFloat(b) - parseFloat(a));
        sortedRatings.forEach(rating => {
            const count = stats.ratingDistribution[rating];
            const percentage = ((count / totalRated) * 100);
            ratingsHTML += `<li class="rating-bar-container" data-rating="${rating}"><div class="rating-bar-label">${createStarsForDisplay(parseFloat(rating))}</div><div class="rating-bar"><div class="rating-bar-fill" style="width: 0%;" data-width="${percentage}%"></div></div><span class="rating-bar-count">${count}</span></li>`;
        });
        ratingsHTML += `</ul></div>`;
    ...
    if (stats.badges) {
        ...
        if (earnedBadges.length > 0) {
            ...
            earnedBadges.slice(-5, -1).reverse().forEach(badge => {
        ...
        allBadges.forEach(badge => {
            ...
            badgesHTML += `<div class="badge-card ${isEarned ? 'earned' : 'locked'}" data-badge-id="${badge.id}"><div class="badge-icon">${badge.icon}</div><div class="badge-text"><span class="badge-name">${badge.name}</span>${!isEarned ? '<span class="badge-locked-text">Kilitli</span>' : ''}</div></div>`;
        });
        badgesHTML += `</div>`;
    ...
    if (badgeContainer) {
        badgeContainer.addEventListener('click', (e) => {
            const card = e.target.closest('.badge-card, .featured-badge, .badge-icon-preview');
            if (card) {
    ...
    if (toggleBtn && badgeGrid) {
        toggleBtn.addEventListener('click', () => {
            ...
            toggleBtn.textContent = badgeGrid.classList.contains('expanded') ? 'Gizle' : 'Tüm Koleksiyonu Gör';
        });
    }

    setTimeout(() => {
        document.querySelectorAll('.rating-bar-fill').forEach(bar => bar.style.width = bar.dataset.width);
    }, 100);
}

export function renderSpecialLists(container, lists, onListClick) {
    container.innerHTML = '<h2 class="text-xl font-bold mb-4 text-gray-200">Özel Listeler</h2>';
...
export function renderListDetail(gridElement, movies, openDetailModalFunction) {
    gridElement.innerHTML = '';
    if (movies.length === 0) {
        gridElement.innerHTML = '<p class="text-gray-500 text-center col-span-full">Bu listede film bulunamadı veya liste yüklenemedi.</p>';
        return;
    }
    movies.forEach(movie => {
        const trendingItem = document.createElement('div');
